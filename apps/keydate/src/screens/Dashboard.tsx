...
import { card, HouseSvg, MoneyInput } from '../components/atoms'
...
export function Dashboard({
  ...
  return (
    <>
      {/* Hero: the keys date */}
      <div style={{ background: C.spruce, borderRadius: 22, padding: '28px 24px 24px', margin: '16px 0', color: '#fff' }}>
        <div
          style={{
            fontSize: 12,
            fontWeight: 600,
        ...
        <div style={{ fontSize: 13.5, color: 'rgba(255,255,255,0.85)', marginTop: 6 }}>
          {fmtShort(plan.target)} {homeTypeLabel(plan.homeType).toLowerCase()} · est. {fmt(dash.payment)}/mo
          mortgage{plan.coBuyer ? ' · 👥 with co-buyer' : ''}
        </div>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11.5, color: C.sub, lineHeight: 1.4 }}>
            {lvl.max
              ...
              : `${lvl.toNext} XP to build the next piece. Lessons +100 · deposits +50 · daily visit +10`}
          </div>
        </div>
      </div>

      {/* Faster paths — appears when the date is 4+ years out */}
      {dash.baseMonths > 48 && <FasterPaths state={state} totalSaved={dash.totalSaved} goal={dash.goal} onUpdatePlan={onUpdatePlan} />}
        ...
        return next ? (
          ...
          >
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: C.spruce, marginBottom: 5 }}>
              Your next lesson · {next.mins} min
            </div>
      ...
      <div style={card}>
        <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 12 }}>Your keyring</div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: earned ? C.goldSoft : C.paper,
                  borderRadius: 12,
                  padding: '10px 4px',
                }}
              >
      ...
      {state.contributions.length > 0 && (
        <div style={{ marginBottom: 18 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: C.sub, marginBottom: 8 }}>Recent deposits</div>
          {state.contributions
            .slice(-4)
            .reverse()
            .map((c, i) => (
...
function FasterPaths({
  ...
  return (
    <div style={{ ...card, background: C.goldSoft, border: `1.5px solid ${C.gold}` }}>
